...
def get_character_names(file):
    ...
    for sentence in file['root']['document']['sentences']['sentence']:
        ...
        for entity in entities:
            if type(entity) is dict and entity.get('NER') is not None and entity['NER'] == 'PERSON':
...
def get_character_mentions(file):
    ...
    if file.get('root') is None or file['root'].get('document') is None or file['root']['document'].get(
            'coreference') is None:
        return characters
    for coreference in file['root']['document']['coreference']['coreference']:
        ...
        for mention in coreference['mention']:
            ...
            if not isinstance(file['root']['document']['sentences']['sentence'],list) or not isValidIndex(file['root']['document']['sentences']['sentence'], int(mention['sentence']) - 1):
                break
            ...
            if mention.get('@representative') is not None and mention.get('@representative') == 'true':
                ...
                if not isinstance(sentence['tokens']['token'],list) or not isValidIndex(sentence['tokens']['token'], int(mention['head']) - 1):
                    break
            ...
            if not isinstance(characters, dict) or not isinstance(characters[token], dict):
                break
...
def get_dependencies(file, mentions, mention_inverse_map, dep_alg='collapsed-ccprocessed-dependencies'):
    # get all types of dependencies
    types = set()
    if (file.get('root') is None
    ...
    for sentence in file['root']['document']['sentences']['sentence']:
        if not isinstance(sentence,dict) or sentence.get(dep_alg) is None or sentence[dep_alg].get('dep') is None:
            continue
    ...
    for t in types:
        for sentence in file['root']['document']['sentences']['sentence']:
            ...
            for dep in sentence[dep_alg]['dep']:
                ...
                character = get_mentioned_character(mention_inverse_map, int(sentence['@id']),
